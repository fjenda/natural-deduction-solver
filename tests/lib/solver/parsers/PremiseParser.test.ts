...
test('PremiseParser parsePremise', () => {
	...
	expect(parsedPremise.tree?.children[0].children[1].type).toBe(NodeType.BINARY_OPERATION);
	expect(parsedPremise.tree?.children[0].children[1].value).toBe(Operator.CONJUNCTION);
	expect(parsedPremise.tree?.children[0].children[1].children.length).toBe(2);
	expect(parsedPremise.tree?.children[0].children[1].children[0].type).toBe(NodeType.CONSTANT);
	expect(parsedPremise.tree?.children[0].children[1].children[0].value).toBe('A');
	expect(parsedPremise.tree?.children[0].children[1].children[1].type).toBe(NodeType.CONSTANT);
	expect(parsedPremise.tree?.children[0].children[1].children[1].value).toBe('B');
