<script lang="ts">
    ...
    import SolverTable from "../../solver/components/solver-table/SolverTable.svelte";
    import { onChangeConclusion, onChangeTheorem } from "../../solver/actions/proofActions";
</script>

<SolverLayout>
    {#if !$solving && $editState === EditState.SOLVER}
        {#each Array.from($solverContent.premises) as _, i}
            <PremiseInputContainer index={i} bind:value={$solverContent.premises[i].value} />
        {/each}
