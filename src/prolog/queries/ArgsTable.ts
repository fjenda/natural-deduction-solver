...
const Queries = {
  ...
  GET: (functor: string, argsCount: number) =>
    `args_table_get(${functor}, ${argsCount}, X).`,
...
export const ArgsTable = {
  ...
  async remove(term: string, argsCount: number, args: string[]) {
    (
      await PrologController.query(Queries.REMOVE(term, argsCount, args))
    ).once();
  },
  ...
  async getMatching(functor: string, argsCount: number) {
    ...
    if (!results) {
      console.warn(
        `No results found for get_matching(${functor}, ${argsCount})`,
      );
      return null;
    }
